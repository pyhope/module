...
def calculate_constants_hp(r_ax, Borb, omega, mu, sigma_r, n_skin_depth=7.0, Rj_boundary=0.0-1.0j, cutoff_index=None):
...
def calculate_constants_lp(r_ax, Borb, omega, mu, sigma_r, n_skin_depth=7.0, Rj_boundary=0.0-1.0j, cutoff_index=0):
    cons_start = time.time()
    Be = Borb
    sigma = sigma_r
    r = r_ax
    ...
    for i in range(r.shape[0]):
        kj_cut[i] = np.sqrt(-1j * 4.0 * np.pi * omega * mu * sigma[i] / c0**2)
...
def magnetic_field_hp(kj, r_ax, nTheta, Cj, Dj, sigma_r):
...
def magnetic_field_lp(kj, r_ax, nTheta, Cj, Dj, sigma_r):
